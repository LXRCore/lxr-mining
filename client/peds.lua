--[[
    🐺 LXR Mining System — client/peds.lua
    ═══════════════════════════════════════════════════════════════════════════════
...
CreateThread(function()
    if next(Config.Peds) then
        for k, v in pairs(Config.Peds) do
            ...
            exports['lxr-core']:createPrompt(k, vector3(v.Coords), Config.Keys.Interact, Lang:t('menu.open') .. v.Label, {
                type = 'client',
                event = 'lxr-mining:client:OpenMenu',
                args = {k},
            })
...
local function DeletePrompt()
    for name, _ in ipairs(Config.Peds) do
        exports['lxr-core']:deletePrompt(name)
    end
    for id, _ in ipairs(Config.MiningLocations) do
        exports['lxr-core']:deletePrompt('Zone_'..id)
    end
end
